class Game {
  ...
  play(){
   ...
   player.update();
    ...
    imageMode(CENTER);
    Player.getPlayerInfo();
    image(gold, 0, -100, 350, 300);
    image(silver, windowWidth/4, -100+windowHeight/10, 325, 270);
    image(bronze, windowWidth/-4, -100+windowHeight/9, 300, 240);
    textAlign(CENTER);
    textSize(30);
    for(var plr in allPlayers ){
      if(allPlayers[plr].place === 1){
        ...
        text("2nd:"+allPlayers[plr].name, windowWidth/4 ,windowHeight/9+73);
      }else if(allPlayers[plr].place === 3){
        text("3rd:"+allPlayers[plr].name, windowWidth/-4 ,windowHeight/10+76);
      }else{
        textSize(20);
        text("Honarary Mention:"+allPlayers[plr].name, 0, 200);
